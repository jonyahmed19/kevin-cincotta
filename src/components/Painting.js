...
import { GetInfo, GetSeries } from "../data/WorkFunctions"
...
const Original = ({ painting, nickname, price, size, sold }) => {
  ...
  return (
    <div>
      {sold && (
        <p>Sorry, this original was sold, but prints are still available.  I also could create something similar for you as a <Link to="/commissions">commissioned work</Link>.</p>
      )}
      {!sold && (<>
...
function Prints({ painting, nickname }) {
  ...
  return <>
    <p>
      Beautiful giclée art prints on lightly textured paper:
    </p>
    <div className="prices">
      {xl && (<>
...
const Painting = (props) => {
  const painting = Works.find(
    (element) => element.nickname === props.nickname
  );
  return (
    <Page {...props}>
      ...
      <div className="splittable">
        ...
        <div className="info">
          ...
          <h2>Questions?</h2>
          <p>
            If you have any doubts or questions, feel free to contact me in whatever way is most convenient.
